...
- Asset-Version auf `2026-06-28-9` erhöht.

## Änderung 2026-06-28-9

Offline-Starter für alle großen Desktop-Systeme ergänzt/aktualisiert:

- `start_lawking_linux.sh`
- `start_lawking_macos.command`
- `start_lawking_windows.bat`

Die Starter starten zuerst einen lokalen HTTP-Server auf `127.0.0.1:8765`, prüfen kurz, ob `browser.html` erreichbar ist, und öffnen erst danach den Browser.
...
## Änderung 2026-06-28-9 Feinschliff

- Manueller Alias ergänzt: `Viehverkehrsverordnung` → `viehverkv_2007`.
- Manueller Alias geprüft: `Berufsqualifikationsfeststellungsgesetz` → `bqfg` ist vorhanden.
- Copyright-Hinweise in Build-/Hilfsskripten auf gizmore vereinheitlicht.
- Browser-Regex-Suche robuster gemacht: statusbehaftete Flags `g`/`y` werden für Suchtests entfernt, damit Treffer nicht durch `lastIndex` flackern.
- Windows-Offline-Starter robuster gemacht: bestehender Port wird erkannt; Python-Aufruf und Log-Umleitung sind sauberer gequotet.
- Asset-Version auf `2026-06-28-9` erhöht.
